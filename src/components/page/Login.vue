<template>
    <div class="login-wrap">
        <img src="../../../static/img/loginBg.png" style="width: 100%;height: 100%;"/>
        <div class="ms-title">
            哈哈镜订货系统
        </div>
        <div class="ms-login">
...
<script>
    export default {
        ...
        created(){
            window.sessionStorage.removeItem('ms_usernatme');
                    ...
                    if (valid) {
                        self.$http.post('/api/user/login',JSON.stringify(self.ruleForm)).then((response) => {
                            ...
                            if (response.data == -1) {
                                ...
                                // 把账号密码存到store里面
                                // this.$store.commit("userInfoMut",self.ruleForm.name);
                                sessionStorage.setItem('ms_usernatme',self.ruleForm.name);
